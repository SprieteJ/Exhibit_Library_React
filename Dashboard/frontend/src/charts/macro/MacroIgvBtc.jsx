...
export default function MacroIgvBtc({ from, to, window: win }) {
  ...
  if (data?.dates?.length) {
    chartData = {
      labels: data.dates,
      datasets: [
        { label: 'IGV (US Software)', data: data.igv, borderColor: '#2471CC', backgroundColor: 'transparent', borderWidth: 1.4, pointRadius: 0, tension: 0.1, yAxisID: 'y', spanGaps: true },
  ...
  return (
    <ChartPanel
      title={`US Software (IGV) vs BTC (${win || 30}d correlation)`}
