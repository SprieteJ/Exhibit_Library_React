...
export default function Btc200dDev({ from, to }) {
  const url = `/api/btc-200d-dev?from=${from}&to=${to}`;
  ...
  if (data?.dates?.length) {
    ...
    chartData = {
      labels: data.dates,
      datasets: [{
        label: 'Deviation %', data: data.deviation,
        backgroundColor: colors, borderColor: colors,
        borderWidth: 0, pointRadius: 0, type: 'bar',
      }],
    };
  ...
  return (
    <ChartPanel
      title="BTC 200-Week MA Deviation"
      source="Source: CoinGecko Pro · % above/below 1400d SMA"
