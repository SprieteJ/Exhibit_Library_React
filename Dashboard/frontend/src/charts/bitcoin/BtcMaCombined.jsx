...
export default function BtcMaCombined({ from, to }) {
  ...
  if (gap.data?.dates?.length) {
    ...
    gapChart = {
      ...
      datasets: [{ label: 'MA Gap %', data: gap.data.gap, backgroundColor: colors, borderColor: colors, borderWidth: 0, pointRadius: 0, type: 'bar' }],
    };
      ...
      summary = (
        <>
          ...
          <div className="perf-item">
            <span style={{ color: lastGap >= 0 ? '#00D64A' : '#EC5B5B', fontWeight: 600 }}>Gap</span>{' '}
            <span className={lastGap >= 0 ? 'pos' : 'neg'}>{lastGap >= 0 ? '+' : ''}{lastGap.toFixed(1)}%</span>
          </div>
        </>
      );
    }
