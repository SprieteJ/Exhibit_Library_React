...
export default function EthMcap({ from, to }) {
  const url = `/api/eth-mcap?from=${from}&to=${to}`;
  ...
  if (data?.dates?.length) {
    const ds = [{
      label: 'ETH Market Cap', data: data.mcap,
    ...
    if (last) summary = <div className="perf-item"><span style={{ color: '#627EEA', fontWeight: 600 }}>Market Cap</span> ${fmtBig(last)}</div>;
  }

  return (
    <ChartPanel title="ETH Market Cap" source="Source: CoinGecko Pro"
      ...
      chartOptions={{
        ...
        plugins: { legend: { display: true, labels: { color: '#888', font: { size: 11 }, boxWidth: 12, filter: item => item.text !== 'ETH Market Cap' } } },
      }}
      summary={summary}
    />
  );
